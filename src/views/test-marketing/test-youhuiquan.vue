  ...
  <div class="app-container">
    ...
    <div class="filter-container" style="border:1px solid #DCDFE6;margin-bottom:20px;height:115px;">
      <div style="width:100%;height:50px;background-color:#F3F3F3;position:relative;margin-bottom:10px;">
        ...
        <div>
          <el-button v-permission="['GET /coupons']" class="filter-item" icon="el-icon-search" @click="handleFilter" style="margin-top:8px;display:inline-block;">查找</el-button>
          <el-button class="filter-item" icon="el-icon-edit" @click="handleCreate" v-permission="['POST /coupon']"style="margin-top:8px;display:inline-block;">添加</el-button>
        </div>
        </div>
      </div>
      <span class="search">请输入优惠券ID</span>
      <el-input
    ...
    >
      ...
      <el-table-column align="center" label="操作" width="100" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" @click="handleUpdate(scope.row)" v-permission="['PUT /coupon/{id}','GET /coupon/{id}']">编辑</el-button>
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      this.$router.push({ path: "/test-marketing/test-youhuiquanadd" });
    },
    handleUpdate(row) {
      this.$router.push({
        path: "/test-marketing/test-youhuiquanedit",
        query: { id: row.id }
      });
    },
